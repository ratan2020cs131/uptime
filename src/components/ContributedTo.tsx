...
const ContributedTo = ({ username, year }: ContributedToProps) => {
  ...
  if (totalRepositories === 0) {
    return <h3 className="text-primary mb-3">Activity overview</h3>;
  }

  return (
    <div>
      <h3 className="text-primary mb-3">Activity overview</h3>
      <div className="flex items-center gap-2 mb-2">
        <IconWrapper icon={repoIcon} alt="repo" size="sm" />
        <h4 className="text-primary text-sm">Contributed to</h4>
      </div>
      <div className="text-sm space-y-1">
          ...
          .map((repo, idx) => (
            <div key={idx}>
              <a
                ...
                className="text-primary-link hover:underline block truncate font-semibold text-[0.8rem] leading-[0.9rem]"
              >
        ...
        {totalRepositories > 3 && (
          <div className="text-primary-dark text-[0.8rem]">
